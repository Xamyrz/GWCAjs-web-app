...
INPUT_BRIDGE_SHIM = """<script>(()=>{const selector='button,[role="button"],input[type="button"],input[type="submit"]',visible=e=>e instanceof HTMLElement&&(e.offsetParent!==null||e.getClientRects().length>0),describe=e=>!e?null:{tag:e.tagName,id:e.id||null,className:typeof e.className==="string"?e.className:null},all=(root=document)=>{const found=[...root.querySelectorAll(selector)];for(const node of root.querySelectorAll("*"))if(node.shadowRoot)found.push(...all(node.shadowRoot));return found},label=e=>String(e.textContent||e.value||e.getAttribute("aria-label")||e.title||"").trim(),moduleRef=()=>window.Module&&window.Module.canvas?window.Module:null,loginButton=()=>all().find(e=>visible(e)&&/log\\s*in/i.test(label(e))),oskInputFor=(kind=\"text\")=>{const mod=moduleRef();if(!mod?.oskInput||typeof mod.oskInput!==\"object\")return null;const input=mod.oskInput[kind];return input instanceof HTMLElement?input:null},pickTarget=()=>{const mod=moduleRef();if(mod?.oskActiveInput instanceof HTMLElement)return mod.oskActiveInput;if(mod?.oskInput&&typeof mod.oskInput===\"object\"){for(const key of[\"text\",\"email\",\"password\",\"number\",\"multiline\"]){const input=mod.oskInput[key];if(input instanceof HTMLElement&&document.activeElement===input)return input}}return mod?.canvas||document.getElementById(\"canvas\")||document.activeElement||document.body},focusTarget=()=>{const target=pickTarget();target?.focus?.({preventScroll:true});return describe(target)},bridge={state(){const mod=moduleRef();return{hasModule:!!mod,hasCanvas:!!mod?.canvas,oskIsActive:!!mod?.oskIsActive,activeElement:describe(document.activeElement),loginButton:describe(loginButton()),oskActiveInput:describe(mod?.oskActiveInput),preferredTarget:describe(pickTarget())}},clickLogin(){const button=loginButton();if(!button)return{clicked:false,button:null};button.click();return{clicked:true,button:describe(button)}},focusPreferredTarget(){return focusTarget()},focusCanvas(){const mod=moduleRef();const canvas=mod?.canvas||document.getElementById(\"canvas\");canvas?.focus?.({preventScroll:true});return describe(canvas)},activateOsk(kind=\"text\"){const mod=moduleRef();const input=oskInputFor(kind);if(!mod||!input)return{activated:false,kind,input:describe(input),state:this.state()};mod.oskActiveInput=input;mod.oskIsActive=true;input.focus?.({preventScroll:true});return{activated:document.activeElement===input,kind,input:describe(input),state:this.state()}},deactivateOsk(){const mod=moduleRef();if(!mod)return this.state();if(mod.oskActiveInput instanceof HTMLElement)mod.oskActiveInput.blur?.();mod.oskActiveInput=null;mod.oskIsActive=false;return this.state()}};window.__GWInputBridge=bridge})();</script>"""
...
class LocalWebAppHandler(SimpleHTTPRequestHandler):
    ...
    def _serve_index_html(self) -> None:
        ...
        content = content.replace(
            ...
            f"{NETWORK_PROXY_SHIM}{STORAGE_SHIM}{HOOK_CONFIG_SHIM}{HOOK_BOOTSTRAP_TAG}{INPUT_BRIDGE_SHIM} <script type=\"module\" src=\"/_astro/Client.astro_astro_type_script_index_0_lang.wgoz1NN0.js\"></script>",
            1,
        )
