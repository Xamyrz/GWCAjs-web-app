...
PROXY_RULES_PATH = os.environ.get("GW_PROXY_RULES", "proxy_overrides.json")
INDEX_FETCH_SNIPPET = (
...
class LocalWebAppHandler(SimpleHTTPRequestHandler):
    ...
    def _serve_index_html(self) -> None:
        ...
        content = content.replace(
            ...
            f"{NETWORK_PROXY_SHIM} <script type=\"module\" src=\"/_astro/Client.astro_astro_type_script_index_0_lang.wgoz1NN0.js\"></script>",
            1,
        )
    ...
    def _proxy_http_request(self) -> None:
        ...
        if parsed_upstream.netloc == "patching.1.arenanetworks.com":
            ...
            if (
                ...
                return
        if (
            parsed_upstream.netloc == "webgate.ncplatform.net"
            and parsed_upstream.path != "/session/create.xml"
            and WEBGATE_SESSION
        ):
